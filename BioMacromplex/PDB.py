...
exteded_chain_ids = [''.join(j) for j in  permutations(ltr, 2)]
        ...
        if cid in self.child_dict:
            for letter in ltr:
                if letter not in self.child_dict:
                    nw_id = letter
                    break
            if nw_id is None and big_id:
                for two_letter in exteded_chain_ids:
                    if two_letter not in self.child_dict:
                        nw_id = two_letter
                        break
            if nw_id is None:
                    ...
                    for atom in residue:
                        if atom_name is None or atom.get_name() in atom_name:
                            if atom.occupancy is not None and atom.temp_factor is not None:
                                out_pdb.write("%-5s%6s %4s %3s %s%4s    %8.3f%8.3f%8.3f%6.2f%6.2f           %s\n" %('ATOM', line_num, atom.name,
                                                            residue.name, chain.id[0], residue.num,round(atom.coords[0],3),
                                                            atom.coords[1], atom.coords[2],atom.occupancy, atom.temp_factor, atom.name[0]))
                            else:
                                out_pdb.write("%-5s%6s %4s %3s %s%4s    %8.3f%8.3f%8.3f\n" % ('ATOM', line_num, atom.name,
                                                            residue.name, chain.id[0], residue.num, round(atom.coords[0], 3),
